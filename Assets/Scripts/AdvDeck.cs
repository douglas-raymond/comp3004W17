...
public class AdvDeck : Deck  {
	...
	public void initDeck(){
		...
		for(int i = 107; i < 108; i ++) { tempCardArr[i] = lib.getCard("galahad"); }
		for(int i = 108; i < 109; i ++) { tempCardArr[i] = lib.getCard("arthur"); }
		for(int i = 109; i < 110; i ++) { tempCardArr[i] = lib.getCard("pellinore"); }
		for(int i = 110; i < 111; i ++) { tempCardArr[i] = lib.getCard("guinevere"); }
		for(int i = 111; i < 112; i ++) { tempCardArr[i] = lib.getCard("iseult"); }
		for(int i = 112; i < 113; i ++) { tempCardArr[i] = lib.getCard("gawain"); }
		for(int i = 113; i < 114; i ++) { tempCardArr[i] = lib.getCard("lancelot"); }
		for(int i = 114; i < 115; i ++) { tempCardArr[i] = lib.getCard("percival"); }
		for(int i = 115; i < 116; i ++) { tempCardArr[i] = lib.getCard("tristan"); }		
